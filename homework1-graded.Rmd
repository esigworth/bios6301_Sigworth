...
exercise <- factor(c('moderate','frequent','some','some',
...
```
Looking at the output, it would appear that gender has the highest effect on length of stay, 
since this variable has the largest coefficient.
```
...
```{r}
stderrors <- sqrt(diag(vcov(summary(mod))))
...
mod.c[,1]/mod.c[,2] # test statistics
```
...
```{r}
pval <- 2*pt(q = 2.917110,df = 7,lower.tail = FALSE)
pval # the p-value for gender
```
...
```{r}
sqred <- residuals(mod)^2
sum(sqred)
deviance(mod)
deviance(mod)==sum(sqred) # the sum of the squared residuals is the deviance
```
...
```{r}
df.residual(mod) # degrees of freedom of residuals
```
...
```{r}
var(men$los) # variance for los for the men
var(women$los) # variance for los for the women
```
...
```{r}
t.test(women$los,men$los) # t test not assuming equal variance
t.test(women$los,men$los,var.equal=TRUE) # t test assuming equal variance
```
```
The model that assumes equal variance produces the same p-value from the model summary.
```
